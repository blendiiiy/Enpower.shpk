...
export function WhatsAppButton() {
  ...
  return (
    <motion.a
      ...
      className="fixed bottom-5 right-4 sm:right-6 z-50 flex h-10 sm:h-11 items-center gap-2 rounded-full bg-gradient-to-r from-brand-700 to-brand-600 px-3 sm:px-3.5 text-white shadow-lg shadow-brand-700/35 hover:from-brand-600 hover:to-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2"
      aria-label="Na shkruaj në WhatsApp"
      title="WhatsApp"
    >
      <span className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-white/20">
      ...
      <span className="hidden sm:inline text-xs font-semibold tracking-wide">WhatsApp</span>
    </motion.a>
  )
}
